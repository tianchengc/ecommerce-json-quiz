...
import { QuizLocaleConfig, QuizQuestion, SelectQuizQuestion, TextareaQuizQuestion } from '@/lib/schemas';
import { useState } from 'react';
import { useRouter } from 'next/navigation';

function isSelectableQuestion(question: QuizQuestion): question is SelectQuizQuestion {
  return question.type === 'single-select' || question.type === 'multi-select';
}

function isTextareaQuestion(question: QuizQuestion): question is TextareaQuizQuestion {
...
export default function QuizForm({ config, locale }: { config: QuizLocaleConfig; locale: string }) {
  ...
  const selectedOptions = isSelectableQuestion(currentQuestion) ? answers[currentQuestion.id] || [] : [];
  const textAnswer = isTextareaQuestion(currentQuestion) ? textAnswers[currentQuestion.id] || '' : '';
  const progress = ((currentQuestionIndex + 1) / questions.length) * 100;

  const handleOptionSelect = (optionId: string) => {
    if (!isSelectableQuestion(currentQuestion)) {
      return;
    }
  ...
  const handleTextareaChange = (value: string) => {
    if (!isTextareaQuestion(currentQuestion)) {
      return;
    }
  ...
  const handleNext = () => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      const formattedAnswers = [
        ...Object.entries(answers).map(([questionId, selectedOptions]) => ({ questionId, selectedOptions })),
        ...Object.entries(textAnswers)
          .filter(([, text]) => text.trim())
          .map(([questionId, textContent]) => ({ questionId, selectedOptions: [] as string[], textContent: textContent.trim() })),
      ];
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 sm:p-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            ...
            {isSelectableQuestion(currentQuestion) ? (
              ...
              <div className="mb-4 sm:mb-6">
                {currentQuestion.helperText && (
                  <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">{currentQuestion.helperText}</p>
                )}
                <textarea
                  ...
                  maxLength={currentQuestion.maxLength || 600}
                  className="w-full min-h-[150px] rounded-lg border border-gray-300 px-3 py-2 text-sm sm:text-base text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                />
                <div className="text-xs text-gray-500 mt-1 text-right">
                  {textAnswer.length}/{currentQuestion.maxLength || 600}
                </div>
              </div>
            )}
